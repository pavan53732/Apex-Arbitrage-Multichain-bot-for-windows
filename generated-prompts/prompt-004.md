...
# Documentation Generation Prompt: Apex Arbitrage Bot Benchmarks
...
**ROUTING MODE:** FLOW-AWARE (Dashboard → Backend → Benchmarks)

**WHAT YOU MUST DO:**
1. ✅ Analyze ALL files under the target folder
2. ✅ Partition by OWNER and FLOW before writing
   - OWNER (benchmarks → quality.md): list ONLY benchmark files as an owner feature in quality.md
   - FLOW (anchored in backend.md): list ONLY backend files; reference Benchmarks counts and links
...
| Backend Logic | core/, engine/, plugins/, utils/, storage/ | backend.md | Services, engines, adapters, non-UI runtime code |
