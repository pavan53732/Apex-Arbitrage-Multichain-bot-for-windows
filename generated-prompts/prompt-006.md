...
- PowerShell will show you 174 files and 34 folders
- You MUST document all 174 files with 20-30 word descriptions
...
**ROUTING MODE:** FLOW-AWARE (Dashboard → Backend → Config)

**WHAT YOU MUST DO:**
1. ✅ Analyze ALL files under the target folder
2. ✅ Partition by OWNER and FLOW before writing
   - OWNER (config → platform.md): list ONLY config files as an owner feature in platform.md
   - FLOW (anchored in backend.md): list ONLY backend files; reference Config counts and links
...
```powershell
try {
    $path = "C:\Users\Pavan pc\Desktop\Apex Arbitrage Multichain bot for windows\Apex-Arbitrage-Multichain-bot-for-windows\Apex Arbitrage Multichain bot\config"
