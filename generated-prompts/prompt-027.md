...
- PowerShell will show you 23 files and 0 folders
- You MUST document all 23 files with 20-30 word descriptions
...
**TARGET FOLDER:** Apex Arbitrage Multichain bot\watchdog
...
- **Target Path:** watchdog/
- **Destination:** backend.md
- **Reason:** Watchdog folder contains monitoring and health check services
- **Feature Type:** OWNER FEATURE
- **NO ANALYSIS REQUIRED** - Route directly to backend.md (Dashboard → Backend → Watchdog)

**WHAT YOU MUST DO:**
1. ✅ Analyze ALL files under the target folder
2. ✅ Partition by OWNER and FLOW before writing
   - OWNER (watchdog → backend.md): list ONLY watchdog files as an owner feature in backend.md
   - FLOW (anchored in backend.md): list ONLY backend files; reference Watchdog counts and links
...
```powershell
try {
    $path = "C:\Users\Pavan pc\Desktop\Apex Arbitrage Multichain bot for windows\Apex-Arbitrage-Multichain-bot-for-windows\Apex Arbitrage Multichain bot\watchdog"
