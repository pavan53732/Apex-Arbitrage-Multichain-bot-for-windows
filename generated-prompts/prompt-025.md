...
- PowerShell will show you 55 files and 8 folders
...
- **Reason:** Vendor folder contains third-party vendor libraries
- **Feature Type:** OWNER FEATURE
- **NO ANALYSIS REQUIRED** - Route directly to platform.md (Dashboard → Backend → Vendor)

**WHAT YOU MUST DO:**
1. ✅ Analyze ALL files under the target folder
2. ✅ Partition by OWNER and FLOW before writing
   - OWNER (vendor → platform.md): list ONLY vendor files as an owner feature in platform.md
   - FLOW (anchored in backend.md): list ONLY backend files; reference Vendor counts and links
...
```powershell
try {
    $path = "C:\Users\Pavan pc\Desktop\Apex Arbitrage Multichain bot for windows\Apex-Arbitrage-Multichain-bot-for-windows\Apex Arbitrage Multichain bot\vendor"
