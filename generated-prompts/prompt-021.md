...
- PowerShell will show you 93 files and 15 folders
- You MUST document all 93 files with 20-30 word descriptions
...
2. ✅ Partition by OWNER and FLOW before writing
   - OWNER (tests → quality.md): list ONLY tests files as an owner feature in quality.md
   - FLOW (anchored in backend.md): list ONLY backend files; reference Tests counts and links
...
```powershell
try {
    $path = "C:\Users\Pavan pc\Desktop\Apex Arbitrage Multichain bot for windows\Apex-Arbitrage-Multichain-bot-for-windows\Apex Arbitrage Multichain bot\tests"
...
| Testing/Performance | test/, *.test.js, *.spec.js, __tests__/ | quality.md | Tests, benchmarks, performance |
| Deployment/Config/Dependencies | deploy/, ci/, docker/, *.yml, config/, *.env, package.json, requirements.txt | platform.md | CI/CD, config, deps |
| Documentation | docs/, *.md, README* | quality.md | Project documentation |
