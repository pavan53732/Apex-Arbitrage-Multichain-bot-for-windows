...
- PowerShell will show you 35 files and 8 folders
- You MUST document all 35 files with 20-30 word descriptions
...
**FULL PATH:** C:\Users\Pavan pc\Desktop\Apex Arbitrage Multichain bot for windows\Apex-Arbitrage-Multichain-bot-for-windows\Apex Arbitrage Multichain bot\manifest
**ANALYSIS MODE:** PROJECT ANALYSIS
**ROUTING MODE:** FLOW-AWARE (Dashboard → Backend → Manifest)

**WHAT YOU MUST DO:**
1. ✅ Analyze ALL files under the target folder
2. ✅ Partition by OWNER and FLOW before writing
   - OWNER (manifest → platform.md): list ONLY manifest files as an owner feature in platform.md
   - FLOW (anchored in backend.md): list ONLY backend files; reference Manifest counts and links
...
| Backend Logic | server/, api/, *.js/*.ts (non-UI) | backend.md | Server-side business logic |
| Testing/Performance | test/, *.test.*, *.spec.*, benchmarks/ | quality.md | Testing and performance/QA assets |
| Documentation | docs/, *.md, README* | quality.md | Project documentation and guides |
| Deployment/CI | deploy/, ci/, docker/, *.yml | platform.md | CI/CD and platform configs |
| Configuration/Secrets | config/, *.env, settings/, *.conf | platform.md | App configuration and secrets |
