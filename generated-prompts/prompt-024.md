...
- PowerShell will show you 65 files and 11 folders
- You MUST document all 65 files with 20-30 word descriptions
...
2. ✅ Partition by OWNER and FLOW before writing
   - OWNER (utils → backend.md): list ONLY utils files as an owner feature in backend.md
   - FLOW (anchored in backend.md): list ONLY backend files; reference Utils counts and links
...
```powershell
try {
    $path = "C:\Users\Pavan pc\Desktop\Apex Arbitrage Multichain bot for windows\Apex-Arbitrage-Multichain-bot-for-windows\Apex Arbitrage Multichain bot\utils"
