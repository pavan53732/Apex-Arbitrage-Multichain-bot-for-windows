...
```powershell
$allFiles = Get-ChildItem -Path "C:\Users\Pavan pc\Desktop\Apex Arbitrage Multichain bot for windows\Apex-Arbitrage-Multichain-bot-for-windows\Apex Arbitrage Multichain bot\config/tokens" -Recurse -File -Force | Select-Object -ExpandProperty FullName
...
Execute using executeBash tool:
```powershell
Get-ChildItem -Path "C:\Users\Pavan pc\Desktop\Apex Arbitrage Multichain bot for windows\Apex-Arbitrage-Multichain-bot-for-windows\Apex Arbitrage Multichain bot\config/tokens" -Recurse -File | Select-Object FullName
```
...
Legacy Path: Apex Arbitrage multi-chain bot/config/tokens
...
4. Update "Recent Completions" to: Prompt 294 (Feature: [Feature Name])
5. Append to Execution Log:
   ```
   Prompt 294: Executed - Added 'Feature: [Feature Name]' to features/[owner].md
   ```
