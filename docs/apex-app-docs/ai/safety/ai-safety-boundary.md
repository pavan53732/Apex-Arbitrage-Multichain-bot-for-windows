...
owner: AI Team
version: 1.0.0
canonical_source: docs/apex-app-docs/ai/safety/ai-safety-boundary.md
related_concepts:
  - CONCEPT-0105
dependencies: []
consumers:
...
| **SB-014** | AI may invoke registered tools within capability grants | Only tools in Tier 1–4 of `../tools/ai-tool-invocation-contract.md` | Tool registry enforces; IPC gateway validates |
...
| **Trust Boundary Filter** | Ensure AI output doesn't attempt cross-domain access | Trust domain enforcement matrix (see `../../security/trust-boundaries.md`) | Block + `security.violation` event |
...
|-----------------|-------------|-------------------|-----------|
| SB-001 | T2 → T1/T0 blocked | IPC secret filter + prompt injection filter | `../../security/trust-boundaries.md` §3 |
| SB-002 | T2 → T1 (advisory only) | Trading Engine rejects AI-initiated trades without approval | `../../security/trust-boundaries.md` §3 |
| SB-003 | T2 → T0/T14 (advisory only) | Config Manager rejects AI mutations | `../../security/trust-boundaries.md` §3 |
| SB-004 | T2 → T*/T4 blocked | IPC gateway | `../../security/trust-boundaries.md` §3 |
| SB-005 | T2 → T5 (provider only) | Network filter | `../../security/trust-boundaries.md` §3 |
| SB-006 | T2 → T3/T5 (filtered output) | Content filter gateway | `../../security/trust-boundaries.md` §3 |
| SB-007 | T2 → T2 (safety gate) | Agent delegation safety check | `../tools/ai-tool-invocation-contract.md` §6 |
